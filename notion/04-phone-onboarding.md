...
## VAPI Configuration — Connecting a Twilio Number

This section covers the exact steps to connect a Twilio number to VAPI so the AI assistant answers inbound calls.

### Prerequisites
- Twilio number provisioned (see options above)
- VAPI account active at [dashboard.vapi.ai](https://dashboard.vapi.ai)
- VAPI assistant created (e.g. "Dental Assistant")

### Step 1 — Get your Twilio credentials

From [Twilio Console](https://console.twilio.com) → Account Info (top of dashboard):
- **Account SID** — copy this
- **Auth Token** — click to reveal, copy this
- **Phone number** — the number you provisioned, in E.164 format (e.g. `+61485004338`)

### Step 2 — Import the number into VAPI

1. VAPI Dashboard → left sidebar → **Phone Numbers**
2. Click **"+"** (top right)
3. Select **Twilio** as the provider
4. Enter your **Account SID** and **Auth Token**
5. Enter the Twilio phone number in E.164 format
6. Click **Import / Create**

VAPI automatically updates the Twilio number's webhook to point to VAPI. You do not need to configure anything in Twilio after this step.

### Step 3 — Assign the assistant (Inbound Settings)

Once the number is imported, click it in the Phone Numbers list. You will see:

| Field | What to do |
|-------|-----------|
| **Inbound Phone Number** | Pre-filled — confirm it matches your Twilio number |
| **Assistant** | Select the assistant (e.g. "Dental Assistant") |
| **Squad** | Leave blank — not needed for single-assistant setup |
| **Workflow** | Leave blank — not needed |
| **Fallback Destination** | Add a real mobile/landline for production. Leave blank for testing. |

> **Fallback Destination note:** Only triggers if the VAPI assistant is unavailable or crashes. For testing, skip it. For a live client, set it to the clinic's main mobile so calls are never lost.

### Step 4 — Outbound Settings

Leave all Outbound Settings blank unless you need to make outgoing calls. Not required for CustomerReach Answer.

### Step 5 — Test

Call the number from any phone. The VAPI assistant should answer within 1–2 seconds. Confirm:
- Assistant greets the caller
- Conversation is transcribed (visible in VAPI → Call Logs)
- n8n receives the end-of-call webhook
- Supabase row is written
- SMS confirmation is sent to the caller
