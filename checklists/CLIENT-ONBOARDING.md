...
- [ ] Duplicate the production Casey assistant → rename: `Casey - Dev`
- [ ] Set Server URL on Casey - Dev to: `.../webhook/vapi-appointment-dev`
...
- [ ] Duplicate again → rename: `Casey - Staging`
- [ ] Set Server URL on Casey - Staging to: `.../webhook/vapi-appointment-staging`
- [ ] Verify production Casey assistant is **unchanged**
...
- **No direct edits to production VAPI assistant** — test on Casey - Dev first
...
- [ ] Log in to [vapi.ai](https://vapi.ai) → Assistants → duplicate production Casey → rename: `Casey - [ClientName]`
...
> **Do NOT change:** system prompt, model settings, or voice settings from the original Casey.
...
- [ ] Go to Table Editor → `user_profiles` → Insert row:
  ...
  | `client_id` | the client's slug (e.g. `riverside-dental`) |
...
- [ ] Call the VAPI number from a mobile — Casey answers immediately
